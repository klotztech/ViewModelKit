    ...
    public class ModuleWeaver : BaseModuleWeaver
    {
        ...
        private void ProcessType(TypeDefinition typeDef)
        {
            if (typeDef.HasCustomAttribute(ViewModelAttributeName, true))
            {
                ...
                {
                    WriteError($"Type {typeDef.FullName} has the {ViewModelAttributeName} attribute but does not implement the INotifyPropertyChanged interface. Type skipped.");
                    return;
                }
                    ...
                    if (attr.AttributeType.FullName == DependsOnAttributeName)
                    {
                        ...
                        if (sourcePropNamesArgs != null)
                        {
                            foreach (var sourcePropNameArg in sourcePropNamesArgs)
                            {
                                ...
                                {
                                    WriteError($"Property {propDef.FullName} has an invalid argument for the DependsOn attribute: {sourcePropNameArg.Value}.");
                                    continue;
                                }
                                ...
                                {
                                    WriteWarning($"Property {propDef.FullName} depends on non-existing property {sourcePropName}.");
                                }
                            }
                        }
            ...
            Dictionary<string, FieldReference> backingFields)
        {
            foreach (var propDef in typeDef.Properties)
            {
                if (propDef.PropertyType.Resolve()?.Is(DelegateCommandTypeName) == true)
                {
                    WriteMessage($"Processing Command: {propDef.FullName}", MessageImportance.High);
                    ...
                    {
                        WriteMessage($"Processing CanExecute: {canExecuteMethod.FullName}", MessageImportance.High);
                            ...
                            if (instr.OpCode == OpCodes.Call || instr.OpCode == OpCodes.Callvirt)
                            {
                                ...
                                if (calledGetMethod != null && instr.Previous?.OpCode == OpCodes.Ldarg_0)
                                {
                                    foreach (var calledPropDef in typeDef.Properties.Where(p => p != propDef))
                                    {
                                        ...
                                        {
                                            WriteMessage($"Found prop ref: {calledPropDef.FullName}", MessageImportance.High);
                                            ...
                                            dependencies.Add(propDef.Name, "A:" + calledPropDef.Name);
                                            break;
                                        }
                                    }
                                }
                            ...
                            if (sourcePropNames != null)
                            {
                                foreach (string sourcePropName in sourcePropNames)
                                {
                                    if (typeDef.Properties.Any(p => p.Name == sourcePropName))
                                    {
                                        ...
                                        dependencies.Add(propDef.Name, "A:" + sourcePropName);
                                    }
                                    else
                                    {
                                        WriteWarning($"Command {propDef.FullName} depends on non-existing property {sourcePropName}.");
                                    }
                                }
                            }
            ...
            bool isValidating)
        {
            ...
            WriteMessage($"dependencies.Values = {string.Join(", ", dependencies.Values)}", MessageImportance.High);
            WriteMessage($"knownAutoProperties = {string.Join(", ", knownAutoProperties)}", MessageImportance.High);
            WriteMessage($"knownCommands = {string.Join(", ", knownCommands)}", MessageImportance.High);
            ...
            {
                ProcessCommandProperties(typeDef, dependencies, commandProperties, backingFields);
            }
        }
            ...
            bool isValidating)
        {
            ...
            WriteMessage($"Processing ViewModel Property: {propDef.FullName}", MessageImportance.High);
            ...
            {
                WriteWarning($"Method signature of {typeDef.FullName}.On{propDef.Name}Changing method is unsupported, method is ignored. Use the IgnoreUnsupportedSignature attribute to avoid this warning.");
            }
            if (changingMethod != null)
            {
            ...
            else if (propDef.Name != IsModifiedPropertyName && propDef.Name != IsLoadedPropertyName)
            {
                    ...
                    isModifiedProperty.SetMethod.IsStatic))
                {
                    ...
                    {
                        WriteWarning($"Property type of {typeDef.FullName}.{IsModifiedPropertyName} property is unsupported, property is ignored. Use the IgnoreUnsupportedSignature attribute to avoid this warning.");
                    }
                    ...
                    isLoadedProperty.GetMethod.IsStatic))
                {
                    ...
                    {
                        WriteWarning($"Property type of {typeDef.FullName}.{IsLoadedPropertyName} property is unsupported, property is ignored. Use the IgnoreUnsupportedSignature attribute to avoid this warning.");
                    }
            ...
            Dictionary<string, FieldReference> backingFields)
        {
            ...
            {
                WriteMessage($"ChangedProperties: {propDef.FullName} => {string.Join(", ", changedProperties)}", MessageImportance.High);
                ...
                if (changedMethod == null &&
                    typeDef.Methods.Any(m => m.Name == $"On{propName}Changed" &&
                        !m.HasCustomAttribute(IgnoreUnsupportedSignatureAttributeName)))
                {
                    WriteWarning($"Method signature of {typeDef.FullName}.On{propName}Changed method is unsupported, method is ignored. Use the IgnoreUnsupportedSignature attribute to avoid this warning.");
                }
                if (changedMethod != null)
                {
                    il.Append(il.Create(OpCodes.Ldarg_0));
            ...
            else
            {
                ...
                {
                    WriteError($"Neither PropertyChanged event field nor OnPropertyChanged method found in type {typeDef.FullName}. Exception ahead…");
                }
            }
            ...
            foreach (string notifyCommandName in notifyCommandNames.Distinct())
            {
                ...
                il.Append(il.Create(OpCodes.Call, refs.DelegateCommandRaiseCanExecuteChangedMethodReference));
            ...
            foreach (var propDef in propDefs)
            {
                ...
                if (executeMethod == null &&
                    typeDef.Methods.Any(m => m.Name == $"On{commandName}" &&
                        !m.HasCustomAttribute(IgnoreUnsupportedSignatureAttributeName)))
                {
                    WriteWarning($"Method signature of {typeDef.FullName}.On{commandName} method is unsupported, method is ignored. Use the IgnoreUnsupportedSignature attribute to avoid this warning.");
                }
                ...
                if (canExecuteMethod == null &&
                    typeDef.Methods.Any(m => m.Name == $"Can{commandName}" &&
                        !m.HasCustomAttribute(IgnoreUnsupportedSignatureAttributeName)))
                {
                    WriteWarning($"Method signature of {typeDef.FullName}.Can{commandName} method is unsupported, method is ignored. Use the IgnoreUnsupportedSignature attribute to avoid this warning.");
                }
                ...
                MethodReference commandConstructor = null;
                if (executeMethod == null)
                {
                    //constructors = null;
                ...
                {
                    MethodReference actionCtor;
                    if (executeMethod.HasParameters)
                    {
                        var commandParameter = executeMethod.Parameters[0];
                        actionCtor = refs.GenericActionConstructor(commandParameter.ParameterType);
                    }
                    else
                    {
                        actionCtor = ModuleDefinition.ImportReference(typeof(Action).GetConstructors().First());
                    }

                    // Create Action delegate object
                    il.Append(il.Create(OpCodes.Ldarg_0));
                    ...
                    {
                        MethodReference funcCtor;
                        if (canExecuteMethod.HasParameters)
                        {
                            var commandParameter = executeMethod.Parameters[0];
                            funcCtor = refs.GenericPredicateFuncConstructor(commandParameter.ParameterType);
                        }
                        else
                        {
                            funcCtor = ModuleDefinition.ImportReference(typeof(Func<bool>).GetConstructors().First());
                        }
                    ...
                    foreach (var dependencyName in dependencies.GetValuesOrEmpty(propDef.Name).Where(dep => dep.StartsWith("A:")))
                    {
                        var depPropDef = typeDef.Properties.FirstOrDefault(p => p.Name == dependencyName.Substring(2));
                        WriteMessage($"Attaching Observer: {depPropDef.FullName}", MessageImportance.High);
#if true
                        // selfConst: Expression.Constant(this, typeof(MyViewModel));
                        il.Append(il.Create(OpCodes.Ldarg_0));
                        il.Append(il.Create(OpCodes.Ldtoken, typeDef));
                        var getTypeFromHandleMethod = typeof(Type).GetMethod("GetTypeFromHandle", new Type[] { typeof(RuntimeTypeHandle) });
                        il.Append(il.Create(OpCodes.Call, ModuleDefinition.ImportReference(getTypeFromHandleMethod)));
                        il.Append(il.Create(OpCodes.Call, refs.ConstantExpressionMethod));

                        // methodInfo: MethodBase.GetMethodFromHandle(methodref(MyViewModel::get_MyProperty))
                        il.Append(il.Create(OpCodes.Ldtoken, depPropDef.GetMethod));
                        var getMethodFromHandleMethod = typeof(System.Reflection.MethodBase).GetMethod("GetMethodFromHandle", new Type[] { typeof(RuntimeMethodHandle) });
                        il.Append(il.Create(OpCodes.Call, ModuleDefinition.ImportReference(getMethodFromHandleMethod)));
                        il.Append(il.Create(OpCodes.Castclass, ModuleDefinition.ImportReference(typeof(System.Reflection.MethodInfo))));

                        // body: Expression.Property(selfConst, methodInfo)
                        il.Append(il.Create(OpCodes.Call, refs.MemberExpressionMethod));

                        // lambda: Expression.Lambda<Func<T>>(body, Array.Empty<ParameterExpression>())
                        var emptyArrayMethod = refs.GenericEmptyArrayMethod(refs.ParameterExpressionType);
                        il.Append(il.Create(OpCodes.Call, emptyArrayMethod));
                        var genericLambdaMethod = refs.GenericLambdaMethod(depPropDef.PropertyType);
                        il.Append(il.Create(OpCodes.Call, genericLambdaMethod));

                        // DelegateCommand.ObservesProperty(lambda)
